...
const compressPDF = async (req, res) => {
  try {
    ...
    const baseUrl = `${protocol}://${host}`; // Construimos la URL base del servidor (ejemplo: "https://backend-encoder-goti.onrender.com") usando el protocolo (http/https) y el host de la petición entrante; de esta forma la URL funciona tanto en desarrollo local como en producción
...
const mergePDF = async (req, res) => {
  try {
    ...
    const host = req.get("host") || "";
    const protocol = (host.includes("localhost") || host.includes("192.168.")) ? req.protocol : "https";
    const baseUrl = `${protocol}://${host}`; // Construimos la URL base del servidor igual que en compressPDF
